...
async function renderGraphic(start, end, isArchive = false, isAutoUpdate = false) {
  ...
  const dataVr1 = await fetchData('vr1', start, end); // Данные для горелки №1
  const dataVr2 = await fetchData('vr2', start, end); // Данные для горелки №2
  ...
  renderChartCombined(
    {
      parameterType: ['vr1', 'vr2'], // Указываем оба типа данных
      ...
      chartTitle: 'График мощности горелок (ПК)',
      start,
      end,
      isArchive,
      isAutoUpdate,
