...
import { getLabelMapping } from './data.js'; // Импортируем getLabelMapping
...
export async function renderChart(options, elements, isDataVisible) {
  ...
  try {
    ...
    // Получаем соответствия меток для текущего устройства
    const labelMapping = getLabelMapping(device);
