...
async function renderChart(options) {
  ...
  try {
    ...
    hideNoDataMessage(); // Скрываем сообщение, если данные есть
            ...
            tooltip: {
              ...
              callbacks: {
                ...
                label: function (tooltipItem) {
                  ...
                  // Проверяем название набора данных для корректного отображения единиц измерения
                  if (datasetLabel === 'Уровень воды в котле-утилизаторе') {
                    return `${datasetLabel}: ${value} мм`;
                  } else if (datasetLabel === 'Исполнительный механизм на котле-утилизаторе') {
    ...
    if (!isAutoUpdate) hidePreloader();
    showNoDataMessage(); // Показываем сообщение при ошибке
  }
}
